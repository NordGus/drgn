  ...
  def self.notify_postmasters_that_post_office_is_not_configured(current_time)
    ...
    BossKey::Postmaster.with_whom_can_be_broadcasted.find_each do |key|
      ...
      broadcast_prepend_to(
        key.holder,
        target: "notice_board",
        partial: "shared/notifications/post_office_not_configured",
        locals: { current_time:, current_character: key.holder }
      )
    end
  end

  def self.notify_postmasters_that_post_office_settings_changed(post_office)
    # We notify all postmasters that the post office is not configured.
    BossKey::Postmaster.with_whom_can_be_broadcasted.find_each do |key|
      # We prepend the notification to the notice board
      broadcast_prepend_to(
        key.holder,
        target: "notice_board",
        partial: "shared/notifications/post_office_settings_changed",
        locals: { current_time: post_office.updated_at, current_character: key.holder }
      )
    end
  end
end
