class Padlock::Invitation::OnIssuedJob < ApplicationJob
  ...
  retry_on StillAliveError, wait: 5.seconds, attempts: :unlimited, report: true
  ...
  def perform(invitation)
    ...
    return :invitation_in_used if invitation.carrier_id.present?
    ...
    fail StillAliveError, "invitation haven't expired yet" if invitation.expires_at > Time.current
