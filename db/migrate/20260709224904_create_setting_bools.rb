class CreateSettingBools < ActiveRecord::Migration[8.1]
  def change
    create_table :setting_bools do |t|
      t.string :type, null: false, comment: "Contains the unique variable name of the setting"
      t.boolean :value, null: false, default: false
      t.boolean :touched, null: false, default: false, comment: "Indicates if the setting has been touched by the user"
      t.belongs_to :mechanic, null: false, foreign_key: { to_table: :mechanics }, comment: "Mechanic to which the setting belongs to"

      t.timestamps
    end

    add_index :setting_bools, :type, unique: true, name: :uniqueness_setting_bools_on_type
    add_index :setting_bools, [ :type, :mechanic_id ], unique: true, name: :uniqueness_setting_bools_on_type_mechanic_id
  end
end
